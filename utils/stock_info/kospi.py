...
def get_kospi_master_dataframe(base_dir):
    ...
    df.rename(columns={"한글명": "한글종목명"}, inplace=True)
    print("Done")
    df.to_csv(f'{os.path.join(base_dir, "kospi_code")}.csv', index=False, encoding="utf-8")  # Save as an Excel file
    return df

get_kospi_master_dataframe(os.path.join(os.getcwd(), "data", "stock_info"))